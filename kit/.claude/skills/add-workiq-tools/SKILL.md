...
hooks:
  ...
  stop:
    - type: command
      command: node "${CLAUDE_PROJECT_DIR}/.a365-kit/hooks/stop/validate-add-workiq-tools.js"
      timeout: 30000
    - type: prompt
      prompt: |
...
| `missing` / `stale` | false       | **Hard stop with a useful message:** *"WorkIQ tools wire MCP servers into an existing agent — there's no agent code in this workspace yet. Run `/make-ai-teammate` (recommended — WorkIQ requires an AI Teammate or OBO agent) first to scaffold and register the agent, then come back here."* Do not proceed. |
| `missing` / `stale` | true        | Tell the user: *"Found existing agent code but no fresh Agent 365 registration. I'll run `a365-setup` now to register it and write the detection cache, then continue here automatically."* **Read** `.a365-kit/skills/a365-setup/SKILL.md` and follow it to completion, then continue to Step 2. |
...
❌  WorkIQ tools do not have a Microsoft-published adapter for ({programmingLanguage}, {agentStack}).
    ...
    Options:
      1. Switch the agent to a supported framework via /make-ai-teammate.
...
3. Load reference patterns:
   - If .NET: **Read** `.a365-kit/skills/add-workiq-tools/references/dotnet-workiq.md`
   - If Node.js: **Read** `.a365-kit/skills/add-workiq-tools/references/nodejs-workiq.md`
   - If Python: **Read** `.a365-kit/skills/add-workiq-tools/references/python-workiq.md`
...
> - .NET: `.a365-kit/skills/add-workiq-tools/references/dotnet-workiq.md`
> - Node.js: `.a365-kit/skills/add-workiq-tools/references/nodejs-workiq.md`
> - Python: `.a365-kit/skills/add-workiq-tools/references/python-workiq.md`
...
1. **Read** `.a365-kit/skills/add-workiq-tools/references/nodejs-workiq.md` — section "Optional: Word @mention notification handling (LangChain — BEST-EFFORT)".
2. **Edit** `src/agent.ts`:
...
1. Tell the user verbatim: *"Skipped. Word tools still work for direct user prompts; only the proactive @mention path is omitted. You can re-enable later by re-running `/add-workiq-tools` — the skill is idempotent."*
...
- **.NET Patterns:** `.a365-kit/skills/add-workiq-tools/references/dotnet-workiq.md`
- **Node.js Patterns:** `.a365-kit/skills/add-workiq-tools/references/nodejs-workiq.md`
- **Python Patterns:** `.a365-kit/skills/add-workiq-tools/references/python-workiq.md`
