...
hooks:
  ...
  stop:
    - type: command
      command: node "${CLAUDE_PROJECT_DIR}/.a365-kit/hooks/stop/validate-make-ai-teammate.js"
      timeout: 30000
    - type: prompt
      prompt: |
...
| `missing`    | false             | **Empty workspace, new-agent path.** Tell the user: *"This is a fresh workspace — I'll scaffold a starter agent from Agent365-Samples first, then run `a365-setup` to register it."* Jump directly to **Phase 0A.5**. If `argFramework` is set, pre-select the matching sample (e.g. `dotnet` → option 1, `dotnet-sk` → option 2, `langchain` → option 3, etc.) and skip the menu. After scaffolding completes, **Read** `.a365-kit/skills/a365-setup/SKILL.md` and follow it to register the new agent — then return to Step 2 below. |
| `missing`    | true              | Tell the user: *"I found existing agent code but no Agent 365 registration. I'll run `a365-setup` now to register it and detect its framework, then continue here automatically."* **Read** `.a365-kit/skills/a365-setup/SKILL.md` and follow it to completion, then return to Step 2 below. |
...
2. **Read** `.a365-kit/skills/a365-setup/SKILL.md` and follow it to completion — `a365-setup` is what writes `.a365-workspace-detection.local.json`.
...
- **Observability composite:**
  - `obs_entry`   = `UseMicrosoftOpenTelemetry` in `Program.cs` (or legacy `AddA365Tracing`)
   - `obs_token` = OBO: `TokenResolver` plus explicit shared-cache wiring (`IExporterTokenCache<AgenticTokenStruct>`), or an existing verified custom resolver; S2S: `ObservabilityTokenService` / `AddAgent365Observability`
...
- **NodeJS** — `.a365-kit/skills/make-ai-teammate/references/nodejs-ai-teammate.md` (see "Required Packages" for the `{agentStack}` variant, and "Dev dependencies")
- **.NET** — `.a365-kit/skills/make-ai-teammate/references/dotnet-ai-teammate.md` (see "Required NuGet Packages")
- **Python** — `.a365-kit/skills/make-ai-teammate/references/python-ai-teammate.md` (see "Required Dependencies")
...
- **If `has_obs_complete = true`** (rows 2, 4, 6, 8 *with* full end-to-end wiring): tell the user verbatim *"Observability already wired end-to-end (entry-point + token resolver + handler-side baggage / scopes) — skipping. Run `/instrument-observability` to reconfigure."* and mark the task complete. Do NOT invoke the sub-skill.
- **If `has_obs_partial = true`** (any signal of obs in the project but at least one of entry / token / handler is missing): **do not skip — recover.** Tell the user verbatim *"Found partial observability wiring (entry-point present, but token resolver and/or message-handler scopes are missing). Completing the wiring now — re-entering `/instrument-observability` to finish what was left half-done."* Then proceed to the same steps as the `has_obs = false` branch below. The sub-skill is idempotent and additive, so partial-recovery is safe.
...
2. **Read** `.a365-kit/skills/instrument-observability/SKILL.md` and follow it now. Observability is part of the AI Teammate package — do not ask whether to add it. The sub-skill's Phase 0.5 will see the pre-populated cache and skip the confirmation prompt automatically.
...
- **If `has_workiq_complete = false && has_workiq_partial = false`** (rows 1, 2, 5, 6 — no manifest yet): ask the user:
  ...
  >
  > - **Yes** → **Read** `.a365-kit/skills/add-workiq-tools/SKILL.md` and follow it in full.
  > - **Skip** → inform the user they can run `/add-workiq-tools` later.
...
**Read** `.a365-kit/skills/make-ai-teammate/references/deploy-pipeline.md`
...
Your agent now has:
  ...
  • ToolingManifest.json  {has_workiq-at-entry || (user picked yes in Phase 9.6)
                              ...
                              : "not created — run /add-workiq-tools to wire WorkIQ tools"}
  ...
  • WorkIQ tools           {has_workiq-at-entry
                              ...
                              : (user picked yes
                                   ...
                                   : "offered, user skipped — run /add-workiq-tools later")}
...
{If runTarget = "local":}
  • Run mode: Local       Agent runs at http://localhost:3978/api/messages.
                          ...
                          When you're ready for production, re-run /make-ai-teammate
...
If you want to push a code change, re-run /make-ai-teammate and pick "Re-publish" at the row 8 sub-question.
```
...
**Node.js patterns:**
- `.a365-kit/skills/make-ai-teammate/references/nodejs-ai-teammate.md`
- `.a365-kit/skills/make-ai-teammate/references/nodejs-notifications.md`

**.NET patterns:**
- `.a365-kit/skills/make-ai-teammate/references/dotnet-ai-teammate.md`

**Python patterns:**
- `.a365-kit/skills/make-ai-teammate/references/python-ai-teammate.md`

**Deploy pipeline (Phase 9.7 — language-agnostic):**
- `.a365-kit/skills/make-ai-teammate/references/deploy-pipeline.md`

**Shared:**
- `.a365-kit/shared/agent-detection.md`
