...
try {
  ...
  if (fs.existsSync(cachePath)) {
    ...
    authMode            = String(cache.authMode || '').toLowerCase();
...
if (authMode === 's2s') {
  process.stdout.write(JSON.stringify({
    ok: true,
    note: 'Work IQ requires a delegated user token; the skill exits without wiring tools in s2s mode.',
  }));
  process.exit(0);
}
...
if (hasLanguageProject && !fs.existsSync(path.join(cwd, '.a365-workspace-detection.local.json'))) {
  issues.push('.a365-workspace-detection.local.json was not written — Phase 0A Step 1 triage was skipped. The skill must run a365-setup (which writes this cache) before any MCP wiring. Re-run /a365-setup, then re-run /add-workiq-tools');
}
