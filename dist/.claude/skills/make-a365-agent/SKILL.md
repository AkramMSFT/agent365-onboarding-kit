...
hooks:
  ...
  stop:
    - type: command
      command: node "${CLAUDE_PROJECT_DIR}/.a365-kit/hooks/stop/validate-make-a365-agent.js"
      timeout: 15000
    - type: prompt
      prompt: |
...
```
   ...
   - If the user selects **option 4 (AI Teammate)** — stop here and tell them:
     > "AI Teammate setup is handled by the `make-ai-teammate` skill. Run `/a365-setup` and select the AI Teammate path, or invoke `make-ai-teammate` directly."
...
**If yes:** **Read** `.a365-kit/skills/instrument-observability/SKILL.md` and follow it.
...
**If yes:** **Read** `.a365-kit/skills/add-workiq-tools/SKILL.md` and follow it.
